...
def norms_sanity_check(poly_norms):
    ...
    norm_sanity = np.dot(
        norm1, np.array(poly_norms.input.points.get_point(1000)) - np.array(
            [63, 63, poly_norms.input.points.get_point(1000)[2]]))
    if norm_sanity < 0:
...
def get_surface_vectors(poly_norms, surf_bfield):
    ...
    parallels = surf_bfield / np.sqrt(np.sum(surf_bfield**2,
                                             axis=1))[:, np.newaxis]
...
def get_the_line(bfield, surf_seeds, n):
    ...
    source = tvtk.PolyData(points=np.array(
...
def update_the_line(the_line, surf_seeds, seed, length):
    """
    Updates the TD line at each time step, while making sure the length is
    fixed
    """
    the_line.source.points = np.array(
        [surf_seeds.get_point(seed), [0.0, 0.0, 0.0]])
    the_line.update()
...
def get_surface_indexes(surf_poly, the_line):
    ...
    for point in the_line.output.points:
        ...
        surf_line_points.append(
            surf_poly.points.get_point(surf_line_index[-1]))
...
class PolyDataWriter(object):
    ...
    def add_point_data(self,
        ...
        if vectors is not None and vector_name is None:
            raise ValueError(
                "If vectors is specified a name must be specified")
        if scalars is not None and scalar_name is None:
            raise ValueError(
                "If scalars is specified a name must be specified")
...
def write_step(file_name, surface, normals, parallels, torsionals, vperp, vpar,
               vphi):
    """
    Write out the surface vectors and velocity components.
    """

    writer = PolyDataWriter(file_name, surface.output)

    writer.add_point_data(
        scalars=vpar, vectors=parallels, scalar_name="vpar", vector_name="par")
...
def get_data(poly_out, name):
    ...
    for i in range(0, poly_out.point_data.number_of_arrays):
